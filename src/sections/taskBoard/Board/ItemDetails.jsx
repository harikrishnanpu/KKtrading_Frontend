...
import { deleteItem, handleTaskboardDialog, useGetBacklogs, useGetTaskboardMaster, useGetUsers } from 'api/taskboard';
...
export default function ItemDetails() {
  ...
  const { taskboardMaster } = useGetTaskboardMaster();
    const { users, usersLoading, usersError } = useGetUsers();
  

  const selectedItem = taskboardMaster?.selectedItem ? taskboardMaster.selectedItem : false;
  // drawer
  const [open, setOpen] = useState(selectedItem !== false);
  const handleDrawerOpen = () => {
    setOpen((prevState) => !prevState);
    handleTaskboardDialog(false);
  };
  ...
  if (selectedItem !== false) {
    ...
    if (selectedData?.commentIds) {
      commentList = [...selectedData.commentIds].reverse().map((commentId, index) => {
        ...
        const profile = users?.find((u) => u._id === commentData?.author);
                return <ItemComment key={index} comment={commentData} profile={profile} itemId={selectedData.id}  />;
      });
    }
  }
