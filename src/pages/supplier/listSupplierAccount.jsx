...
// Material-UI Dialog Imports for Modal
...
const SupplierAccountList = () => {
  ...
  // Global filtering/sorting states
  ...
  const fetchAccounts = async () => {
    ...
    try {
      ...
      // Set default selection (from localStorage or select all)
  ...
  // Apply additional filters (payment status, bill amount, pending amount)
  ...
  // Sort the filtered accounts based on sortConfig
  ...
  // PDF Generation for a supplier account
  const generatePDF = (account) => {
    ...
    doc.text(
      `Total Bill Amount: ₹${account.totalBillAmount.toFixed(2)}`,
      14,
      56
    );
    doc.text(`Paid Amount: ₹${account.paidAmount.toFixed(2)}`, 14, 64);
    doc.text(
      `Pending Amount: ₹${account.pendingAmount.toFixed(2)}`,
      14,
      72
    );
    doc.text(
  ...
  // Handle Removing a Supplier Account
  const handleRemove = async (id) => {
    if (
      window.confirm(
        "Are you sure you want to delete this supplier account?"
      )
    ) {
      try {
  ...
  // Handle Viewing Account Details (opens the modal)
  const handleView = (account) => {
    // Reset modal filters if any (for bills and payments)
    setBillSearchQuery("");
    setBillDateFrom("");
    setBillDateTo("");
  ...
  // Sorting handler for the main table
  ...
  // Toggle supplier selection in the sidebar
  ...
  // -------------------
  // Filters for Bills and Payments in the Modal
  // -------------------
        ...
        <tbody>
          {skeletonRows.map((row) => (
            <tr key={row} className="hover:bg-gray-100 divide-y divide-x">
              <td className="px-4 py-2"><Skeleton height={10} /></td>
              <td className="px-2 py-2"><Skeleton height={10} /></td>
              <td className="px-2 py-2"><Skeleton height={10} /></td>
              <td className="px-2 py-2"><Skeleton height={10} /></td>
              <td className="px-2 py-2"><Skeleton height={10} /></td>
              <td className="px-2 py-2"><Skeleton height={10} /></td>
    ...
    <div className="flex flex-col md:flex-row min-h-screen bg-gray-50">
      ...
      >
        <h2
          ...
          className="text-md font-bold text-red-600 mb-4 text-center cursor-pointer"
        >
          Filter Accounts
        </h2>

        {/* Search by Supplier ID/Supplier Name */}
        <div className="mb-4">
          <input
            type="text"
      ...
      <div className="flex-1 p-4 md:p-6 lg:p-8">
        ...
        {/* Accounts Table / Loading Skeleton */}
        {loading ? (
          renderSkeleton()
              ...
              <div>
                <p className="text-sm font-semibold">
                  ...
                  <span className="text-gray-900 font-bold">
                    {selectedAccount?.sellerName}
                  </span>
                </p>
                <p className="text-sm font-semibold">
                  Supplier Address:{" "}
                  <span className="text-gray-900 font-bold">
                    {selectedAccount?.sellerAddress}
                  </span>
                </p>
                <p className="text-sm font-semibold">
                  Total Bill Amount:{" "}
                  ...
                  <tr>
                    ...
                    >
                      ...
                      {paymentSortConfig.key === "amount" && (
                        <i
                          className={`fa fa-sort-${
                            paymentSortConfig.direction === "asc" ? "asc" : "desc"
                          } ml-1`}
                        ></i>
                      )}
                    </th>
