...
export default function Dashboard() {
  ...
  const fetchPieChart = async () => {
    try {
      const response = await api.get('/api/chart/invoice/dashboard/pie-chart');
  ...
  async function fetchRecentInvoices() {
    try {
      const response = await api.get('/api/chart/invoice/dashboard/recent');
    ...
    <>
      ...
      <Grid container spacing={2.5}>
        ...
        <Grid item sm={6} md={4} xs={12}>
          <InvoiceUserList invoices={invoices} loading={loading} error={error} />
        </Grid>
        <Grid item sm={6} md={4} xs={12}>
