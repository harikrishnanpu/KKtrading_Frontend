  ...
  const fetchDashboardData = async (period) => {
    try {
      const response = await api.get(
        `/api/chart/purchase/dashboard/summary?period=${period}`
      );
    ...
    async function fetchRecentInvoices() {
      try {
        const response = await api.get('/api/chart/purchase/dashboard/recent');
    ...
    const fetchPieChart = async () => {
      try {
        const response = await api.get('/api/chart/purchase/dashboard/pie-chart');
    ...
    <>
      ...
      <Grid container spacing={2.5}>
        ...
        <Grid item sm={6} md={4} xs={12}>
          <InvoiceUserList invoices={invoices} error={error} loading={loading} />
        </Grid>
        <Grid item sm={6} md={4} xs={12}>
          <InvoicePieChart loading={loading} series={series} />
        </Grid>
