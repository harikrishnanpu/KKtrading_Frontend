     ...
     {!driverPage  && <p className="text-sm font-bold mb-4 text-gray-600 text-center">Low Stock & Deliveries</p> }
      ...
      {!adminPage && <div className="border border-gray-200 rounded-lg p-4">
          ...
          {billings.length === 0 ? (
            ...
            billings.map((bill) => (
              <div onClick={()=> navigate(`/invoice/details/${bill._id}`)} key={bill.invoiceNo} className="flex cursor-pointer hover:bg-gray-100 rounded-md justify-between items-center p-3 border-b border-gray-200">
                <div>
